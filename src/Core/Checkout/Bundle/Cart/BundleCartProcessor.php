...
namespace Lesha\BundleExample\Core\Checkout\Bundle\Cart;
...
class BundleCartProcessor implements CartProcessorInterface, CartDataCollectorInterface
{
    ...
    public const ARTIKUL = 'bundle-artikul';
    public const DATA_KEY = 'swag_bundle-data';
    /**
     ...
     */
    private $artikulPriceCalculator;
    ...
    public function __construct(
        ...
        $this->artikulPriceCalculator = $artikulPriceCalculator;
    ...
    private function createDiscount(BundleEntity $bundleData, SalesChannelContext $context): ?LineItem
    {
        ...
        $artikul = new LineItem(
            $bundleData->getId() . '-artikul',
            self::ARTIKUL,
            $bundleData->getId()
        );
    ...
    {
        $artikul = $this->getArtikul($bundleLineItem);
        ...
        switch (\get_class($priceDefinition)) {
            ...
            default:
                throw new \RuntimeException('Invalid artikul');
        }
